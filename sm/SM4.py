...
def _message_padding(message, rule = SM4_PAD_PBOC):
    if len(message) % 2 != 0:
...
class SM4(object):
    ...
    def sm4_encrypt(self, message, enc_mode, padding_rule):
        '''
        message  待加密数据
        enc_mode 加密模式   ECB/CBC
        padding_rule 明文填充规则   PAD_PBOC/PAD_UNION
        '''
        try:
            M = _message_padding(message, padding_rule)
        except Exception,e:
            raise Exception(e)
        C = ''
        if enc_mode == SM4_ECB:
